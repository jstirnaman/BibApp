...
#
class Sword2Client::ServiceDocParser
  ...
  def initialize
    # create a parsed service doc object
    @parsed_service_doc = Sword2Client::ParsedServiceDoc.new
  end
  ...
  def text text
    ...
    if @curr_collection and @curr_tag_name and !@curr_tag_name.empty? and @curr_tag_name!=COLLECTION_TAG
        ...
        # (e.g. collection['title'] = "Parsedcollection Title",
        #       collection['collectionPolicy'] = "Parsedcollection License", etc.)
    ...
    # then we've found the repository.rb's name
        ...
        #capture the repository.rb's name
  ...
  #Convert an XML tag to a valid property name for a Parsedcollection
