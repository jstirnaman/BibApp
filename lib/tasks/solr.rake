...
#        sleep(5)
  ...
  desc 'Refresh Solr index. Argument CORE_N= specifies which core to reindex.'
  task :refresh_index => :environment do
    puts "\nRe-indexing core #{SOLR_CORE}\n\n"    
  ...
  desc 'Refresh and Swap Solr index. Argument CORE_N= specifies which core to reindex.'
  task :refresh_swap_index => :environment do
    ...
    begin
      ...
      #Always swap core1 with core2
      n.request_head("/solr/admin/cores?action=SWAP&core=#{Rails.env}-core1&other=#{Rails.env}-core2").value
    end
    ...
    puts "Finished indexing!  Total indexing time: #{time}"
    puts "Remember to restart Delayed Job!" 
  end  
end